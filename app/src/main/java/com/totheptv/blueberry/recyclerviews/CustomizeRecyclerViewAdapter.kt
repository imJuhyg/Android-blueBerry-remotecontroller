...
import com.totheptv.blueberry.recyclerviews.items.CustomizeRecyclerViewItem
...
class CustomizeRecyclerViewAdapter(private val context: Context) : RecyclerView.Adapter<CustomizeRecyclerViewAdapter.ViewHolder>() {
    private val customizeRecyclerViewItems by lazy { ArrayList<CustomizeRecyclerViewItem>() }
    ...
    override fun onBindViewHolder(viewHolder: ViewHolder, position: Int) {
        val item = customizeRecyclerViewItems[position]
        viewHolder.apply {
    ...
    override fun getItemCount() = customizeRecyclerViewItems.size
    ...
    fun addItem(customizeName: String, deviceImage: Drawable?, deviceName: String, deviceAddress: String) {
        ...
        customizeRecyclerViewItems.add(item)
        notifyDataSetChanged()
    }

    fun clear() {
        customizeRecyclerViewItems.clear()
        startOffsetValue = 0
        notifyDataSetChanged()
    }
}